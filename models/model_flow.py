...
class Model_flow(nn.Module):
    def __init__(self, freeze, num_scales):
    ...
    def inference_flow(self, img1, img2):
        ...
        optical_flow = self.pwc_model(feature_list_1, feature_list_2, img_hw)
        return optical_flow
...
if __name__ == '__main__':
    model = Model_flow(freeze=True, num_scales=1).to(device)
    weights = torch.load("pretrained/kitti_odo.pth")
    #print(weights['model_state_dict'].keys())
    model_dict = model.state_dict()
    print(model_dict.keys())
    pretrained_dict = {}
    for k in model_dict.keys():
        for v in weights['model_state_dict'].keys():
            if k in v:
                pretrained_dict[k]=weights['model_state_dict'][v]
    #print(pretrained_dict.keys())


    model.load_state_dict(pretrained_dict)
    #torch.save(model.state_dict(),"kitti_flow.pth")
    model.eval()
    print('Model Loaded.')
    tenFirst = torch.FloatTensor(np.ascontiguousarray(
        np.array(PIL.Image.open("test/1.jpg"))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (
                    1.0 / 255.0))).unsqueeze(0).to(device)
    tenSecond = torch.FloatTensor(np.ascontiguousarray(
        np.array(PIL.Image.open("test/2.jpg"))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (
                    1.0 / 255.0))).unsqueeze(0).to(device)

    show_flow(model.inference_flow(tenFirst, tenSecond)[0])
